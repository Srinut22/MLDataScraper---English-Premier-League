...
team_and_keys = {"Tottenham-Hotspur": ["361ca564", 0], "Manchester-City": ["b8fd03ef", 1], "Liverpool": ["822bd0ba", 2], "Arsenal": ["18bb7c10", 3], "Aston-Villa": ["8602292d", 4], "Newcastle-United": ["b2b47a98", 5], "Brighton-and-Hove-Albion": ["d07537b9", 6], "Manchester-United": ["19538871", 7],
...
def extract_info(parsed_code):
    ...
    date_elements = [
        th_element for th_element in date_elements if th_element.find("a")]
    dates = []
    calculation_dates = []
    ...
    goals_against_elements = parsed_code.find_all(
        "td", {"data-stat": "goals_against"})
    ...
    for date_element, goals_for_element, goals_against_element, opponent_element, venue_element in zip(date_elements, goals_for_elements, goals_against_elements, opponent_elements, venue_elements):
        ...
        opponent = opponent_element.a.get_text(
        ) if opponent_element.a else opponent_element.get_text()
        ...
        if opponent in team_and_keys.keys():
            ...
            if type(goals_for) != int and '(' and ')' in goals_for:
                calculation_goals_for, calculation_penalties_for = getGoalsAndPenalties(
                    goals_for)
            else:
            ...
            if type(goals_against) != int and '(' and ')' in goals_against:
                calculation_goals_against, calculation_penalties_against = getGoalsAndPenalties(
                    goals_against)
            else:
            ...
            calculation_penalties_against_values.append(
                calculation_penalties_against)
...
def getGameHistory(team_name):
    years = [f"{year}-{year + 1}" for year in range(2023, 2000, -1)]
    ...
    for year in years:
        HTML_markup = requests.get(
            f"https://fbref.com/en/squads/{team_and_keys[team_name][0]}/{year}/{team_name}-Stats").text
    ...
    finished_match_data['Match Result'] = finished_match_data.apply(
        calculate_match_result, axis=1)
...
def calculate_match_result(row):
    ...
    if goals_for_value > goals_against_value:
        ...
        goals_for_in_brackets = int(goals_for.split(
            ' ')[1].strip('()')) if '(' in str(goals_for) else 0
        goals_against_in_brackets = int(goals_against.split(
            ' ')[1].strip('()')) if '(' in str(goals_against) else 0
...
def get_future_matches(selected_team):
    HTML_markup = requests.get(
        f"https://fbref.com/en/squads/{team_and_keys[selected_team][0]}/2023-2024/{selected_team}-Stats").text
    parsed_code = BeautifulSoup(HTML_markup, 'html.parser')
    date_elements = parsed_code.find_all("th", {"data-stat": "date"})
    dates = []
    ...
    for date_element, goals_for_element, goals_against_element, opponent_element, venue_element in zip(date_elements, goals_for_elements, goals_against_elements, opponent_elements, venue_elements):
        ...
        venue = venue_element.get_text()
        if opponent == "West Ham":
            opponent = "West Ham United"
        elif opponent == "Tottenham":
            opponent = "Tottenham Hotspur"
        elif opponent == "Wolves":
            opponent = "Wolverhampton Wanderers"
        elif opponent == "Brighton":
...
def predict_values(match_data):
    ...
    training_match_data = match_data.drop(
        columns=["Goals For", "Goals Against", "Penalties For", "Penalties Against", "Match Result"])
    goals_for_predictor.fit(training_match_data, match_data["Goals For"])
    goals_against_predictor.fit(
        training_match_data, match_data["Goals Against"])

    return goals_for_predictor, goals_against_predictor


def extract_goals_and_penalty_info(penalty):
    if '(' in penalty:
        opening_bracket_index = penalty.index('(')
        closing_bracket_index = penalty.index(')')
        goals = int(penalty[:opening_bracket_index])
        penalties = int(
            penalty[opening_bracket_index + 1: closing_bracket_index])
    else:
        penalties = 0
        goals = int(penalty)
    return goals, penalties
...
def opponent_to_number(opponent):
    if opponent == "West Ham":
        opponent = "West Ham United"
    elif opponent == "Tottenham":
        opponent = "Tottenham Hotspur"
    ...
    if opponent in team_and_keys:
        return int(team_and_keys[opponent][1])


def venue_to_number(venue):
    if venue == "Home":
        return 2
    elif venue == "Away":
        return 0
...
def make_ML_ready(data):
    calculation_match_data = pd.DataFrame()
    calculation_match_data["Date"] = data["Date"].apply(convertDate)
    result = data["Goals For"].apply(extract_goals_and_penalty_info)
    calculation_match_data["Goals For"], calculation_match_data["Penalties For"] = zip(
        *result)
    result = data["Goals Against"].apply(extract_goals_and_penalty_info)
    calculation_match_data["Goals Against"], calculation_match_data["Penalties Against"] = zip(
        *result)
    calculation_match_data["Opponents"] = data["Opponents"].apply(
        opponent_to_number)
    calculation_match_data["Match Result"] = data["Match Result"].apply(
        result_to_number)
    calculation_match_data["Venue"] = data["Venue"].apply(venue_to_number)

    # Filtering out rows where "Opponents" column is None
    calculation_match_data = calculation_match_data.dropna(subset=[
                                                           "Opponents"])

    return calculation_match_data


def process_input(selected_team):
    if selected_team in team_and_keys.keys():
        finished_match_data = getGameHistory(
            selected_team)
        calculation_match_data = make_ML_ready(finished_match_data)
        st.dataframe(calculation_match_data)
        goals_for_predictor, goals_against_predictor = predict_values(
            calculation_match_data)
        future_matches = get_future_matches(selected_team)
        training_future_matches = pd.DataFrame()
        training_future_matches["Date"] = future_matches["Date"].apply(
            convertDate)
        training_future_matches["Opponents"] = future_matches['Opponents'].map(
            lambda team: team_and_keys.get(team, [None, None])[1])
        training_future_matches["Venue"] = future_matches["Venue"].apply(
            getVenue)
        future_matches['Goals For'] = goals_for_predictor.predict(
            training_future_matches)
        future_matches['Goals Against'] = goals_against_predictor.predict(
            training_future_matches)
        future_matches['Match Result'] = future_matches.apply(
            calculate_match_result, axis=1)
...
def main():
    ...
    with st.form("team_selector_form"):
        ...
        selected_team = st.text_input(
            "Enter a Premier League team [Ensure the beginning of each word is Capitalized (Eg: Tottenham Hotspur not tottenham hotspur)]:")
